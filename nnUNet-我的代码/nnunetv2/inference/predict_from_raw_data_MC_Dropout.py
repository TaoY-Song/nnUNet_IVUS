...
class nnUNetPredictor(object):
    def __init__(self,
        ...
        # 新增不确定性参数
        self.uncertainty_threshold = 0.37  # 不确定性阈值
        self.max_retries = 2  # 最大重试次数设为3次
        self.default_tile_step = 0.8
        self.default_sigma = 0.9
        self.current_sigma = self.default_sigma
        self.adaptive_params = {
            'tile_step_size': [0.9, 0.6],  # 与max_retries匹配
            'gaussian_sigma': [0.8, 0.7]
        }

        # MC Dropout参数
        self.mc_samples = 20  # MC采样次数
        self.mc_dropout_prob = 0.5  # Dropout概率
        self.current_retry = 0

        assert len(self.adaptive_params['tile_step_size']) >= self.max_retries, "tile_step_size参数不足"
        assert len(self.adaptive_params['gaussian_sigma']) >= self.max_retries, "gaussian_sigma参数不足"
    ...
    def predict_from_data_iterator(self,
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                case_id = os.path.basename(ofile) if ofile else str(id(data))

                # 初始化案例参数
                if case_id not in case_parameters:
                    case_parameters[case_id] = {
                        'current_retry': 0,
                        'best_prediction': None,
                        'best_uncertainty': float('inf'),
                        'current_params': {
                            'tile_step': self.tile_step_size,
                            'gaussian_sigma': getattr(self.configuration_manager, 'gaussian_sigma', 1.0)
                        }
                    }

                # 主预测循环
                while case_parameters[case_id]['current_retry'] <= self.max_retries:
                    current_retry = case_parameters[case_id]['current_retry']


                    # === 修改参数设置逻辑 ===
                    if current_retry == 0:
                        print(f"\n=== Case {case_id} Attempt Default Parameters  ===")
                        # 第一次尝试使用原版参数
                        new_tile_step = self.default_tile_step  # 原版tile_step_size
                        new_sigma = self.default_sigma  # 原版sigma
                    else:
                        print(f"\n=== Case {case_id} Retry {current_retry}/{self.max_retries} ===")  # 重试次数为max_retries
                        # 后续重试使用adaptive_params中的参数（索引从0开始）
                        new_tile_step = self.adaptive_params['tile_step_size'][current_retry - 1]
                        new_sigma = self.adaptive_params['gaussian_sigma'][current_retry - 1]
                        

                    print(f"Params: tile_step={new_tile_step:.3f}, sigma={new_sigma:.3f}")

                    # 设置当前参数
                    self.tile_step_size = new_tile_step
                    self.current_sigma = new_sigma  # 存储当前sigma供后续使用

                    try:
                        # 执行MC采样（修改为最少2次采样）
                        self._enable_mc_dropout(True)
                        all_predictions = []
                        for _ in range(max(2, self.mc_samples)):  # 至少2次采样确保数值稳定性
                            pred_logits, _ = self.predict_logits_from_preprocessed_data(data)  # 明确解包元组
                            all_predictions.append(pred_logits)
                        mean_prediction = torch.mean(torch.stack(all_predictions), dim=0)

                        # 计算不确定性
                        current_uncertainty = self.compute_uncertainty(mean_prediction)
                        print(f"Uncertainty: {current_uncertainty:.4f} (Threshold: {self.uncertainty_threshold:.4f})")

                        # 导出结果


                        # 更新最佳结果
                        if current_uncertainty < case_parameters[case_id]['best_uncertainty']:
                            case_parameters[case_id]['best_uncertainty'] = current_uncertainty
                            case_parameters[case_id]['best_prediction'] = mean_prediction
                            print("*** Update and export best prediction ***")
                            if ofile is not None:
                                export_pool.starmap_async(
                                    export_prediction_from_logits,
                                    ((mean_prediction, properties, self.configuration_manager,
                                      self.plans_manager, self.dataset_json, ofile, save_probabilities),)
                                )

                        # 检查终止条件
                        if current_uncertainty < self.uncertainty_threshold:
                            print("✅ Below threshold, accept prediction")
                            break
                        else:
                            # 参数调整逻辑
                            if current_retry < self.max_retries:
                                new_params = {
                                    'tile_step': self.adaptive_params['tile_step_size'][current_retry],
                                    'gaussian_sigma': self.adaptive_params['gaussian_sigma'][current_retry]
                                }
                                print(f"🔄 Adjusting params: tile_step={new_params['tile_step']:.3f}, sigma={new_params['gaussian_sigma']:.3f}")
                                # case_parameters[case_id]['current_params'] = new_params
                                case_parameters[case_id]['current_retry'] += 1
                            else:
                        ...
                        if 'CUDA out of memory' in str(e):
                            print("⚠️ OOM detected, reducing parameters")
                ...
                # 导出结果
                final_prediction = case_parameters[case_id]['best_prediction']
                if ofile is not None:
                    export_pool.starmap_async(
                        export_prediction_from_logits,
                        ((final_prediction, properties, self.configuration_manager,
                          self.plans_manager, self.dataset_json, ofile, save_probabilities),)
                    )
    ...
    def _enable_mc_dropout(self, enable: bool = True):
        ...
        def apply_mc_dropout(module):
            if isinstance(module, (nn.Dropout, nn.Dropout2d, nn.Dropout3d)):
                module.train()  # 保持训练模式
                module.p = self.mc_dropout_prob  # 设置概率

        if enable:
            self.network.apply(apply_mc_dropout)
        else:
            # 恢复原始状态
            def restore_dropout(module):
                if isinstance(module, (nn.Dropout, nn.Dropout2d, nn.Dropout3d)):
                    module.eval()

            self.network.apply(restore_dropout)


    def compute_uncertainty(self, logits: torch.Tensor, eps: float = 1e-6) -> float:
        probabilities = torch.softmax(logits, dim=1)
        entropy = -torch.sum(probabilities * torch.log(probabilities + eps), dim=1)
        return torch.mean(entropy).item()*0.1  # 添加与训练相同的缩放因子
    ...
    def predict_single_npy_array(self, input_image: np.ndarray, image_properties: dict,
        ...
        if self.verbose:
            print('predicting')
        predicted_logits, _ = self.predict_logits_from_preprocessed_data(dct['data']).cpu()
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                prediction += torch.flip(self.network(torch.flip(x, axes), torch.flip(x, axes)), axes)
    ...
    def _internal_predict_sliding_window_return_logits(self,
        ...
        try:
            ...
            #原来的计算高斯的代码，sigma_scale被固定为0.125
            ...
            if self.use_gaussian:
                ...
                sigma = min(2.0, max(0.5, sigma))  # 添加边界保护

                gaussian = compute_gaussian(tuple(self.configuration_manager.patch_size), sigma_scale=sigma / 8,
                                                 value_scaling_factor=10,
                                                 device=results_device)
            else:
                gaussian = 1
