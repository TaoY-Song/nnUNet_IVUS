...
class nnUNetPredictor(object):
    ...
    def predict_from_data_iterator(self,
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                prediction_raw, uncertainty_val = self.predict_logits_from_preprocessed_data(data)
                prediction = prediction_raw.cpu() # Keep prediction on CPU

                # ADDED: Print uncertainty
                if ofile is not None:
                    print(f'Uncertainty (entropy) for {os.path.basename(ofile)}: {uncertainty_val:.4f}')
                else:
                    print(f'Uncertainty (entropy) for image of shape {data.shape}: {uncertainty_val:.4f}')
    ...
    def predict_single_npy_array(self, input_image: np.ndarray, image_properties: dict,
        ...
        if self.verbose:
            print('predicting')
        # MODIFIED: Handle two return values
        predicted_logits_raw, uncertainty_val = self.predict_logits_from_preprocessed_data(dct['data'])
        predicted_logits = predicted_logits_raw.cpu() # Keep prediction on CPU

        # ADDED: Print uncertainty
        if self.verbose: # Only print if verbose, or could be unconditional
            print(f'Uncertainty (entropy) for single_npy_array: {uncertainty_val:.4f}')
    ...
    def compute_uncertainty(self, logits: torch.Tensor, eps: float = 1e-6) -> float:
        ...
        return torch.mean(entropy).item() * 0.1
