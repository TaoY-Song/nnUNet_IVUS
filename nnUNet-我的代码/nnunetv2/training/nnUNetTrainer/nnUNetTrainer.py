...
from nnunetv2.inference.predict_from_raw_data_MC_Dropout import nnUNetPredictor
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.mc_samples = 5       # MC采样次数
        self.mc_dropout_prob = 0.5 # 保持与dropout层一致
        self.uncertainty_threshold = 0.3
        
        # 在初始化时扩展日志指标
        self.logger.my_fantastic_logging.update({
            'train_uncertainty': [],
            'val_uncertainty': [],
            'best_train_uncert': None,
            'best_val_uncert': None
        })
    ...
    def _build_loss(self):
        if self.label_manager.has_regions:
            loss = DC_and_BCE_loss({},
                                  ...
                                  weight_factors=getattr(self, 'deep_supervision_weights', None))
        else:
    ...
    def train_step(self, batch: dict) -> dict:
        data = batch['data']
        previous_data = batch['previous_data']
        target = batch['target']
        ...
        self._set_mc_dropout_mode(enable=False)  # 确保使用标准dropout
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            # del data
            l = self.loss(output, target) + self._uncertainty_regularization(output)
            ...
            with torch.no_grad():
                if isinstance(output, list):
                    ...
                    main_output = output
                prob = torch.softmax(main_output, 1)
        ...
        if self.grad_scaler is not None:
            ...
            self.optimizer.step()
        #######添加了uncertainty    
        return {'loss': l.detach().cpu().numpy(), 'uncertainty': uncertainty}#&&&&&&&&&&&&&&&&&&&&&&&&&&&&
    ...
    def on_train_epoch_end(self, train_outputs: List[dict]):
        ...
        if 'uncertainty' in outputs:
            uncertainties = np.array(outputs['uncertainty']).astype(float)
            if self.is_ddp:
                uncertainties_tr = [None for _ in range(dist.get_world_size())]
                dist.all_gather_object(uncertainties_tr, uncertainties)
                uncertainty_here = np.concatenate(uncertainties_tr).mean()
            else:
                uncertainty_here = np.mean(uncertainties)
            
            self.logger.log('train_uncertainty', uncertainty_here, self.current_epoch)
            # self.print_to_log_file(f'Train Uncertainty: {uncertainty_here:.4f}')
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        # 统一目标处理
        if isinstance(target, list):
            main_target = [t.to(self.device) for t in target][0]  # 取首层目标
        else:
            main_target = target.to(self.device)
            # 统一目标处理（关键修改）
    
        if isinstance(target, list):
            # 深度监督模式下取首层目标
            processed_target = target[0].to(self.device)
        else:
            processed_target = target.to(self.device)
        
        # 防御性检查
        assert isinstance(processed_target, torch.Tensor), f"目标必须为张量，实际类型: {type(processed_target)}"
        assert processed_target.ndim in [4, 5], f"无效目标维度: {processed_target.shape}"

    
        outputs = []
        for _ in range(self.mc_samples):
            output = self.network(data, previous_data)
            outputs.append(output if isinstance(output, (list, tuple)) else [output])
    
        # 计算各层平均预测
        num_layers = len(outputs[0])
        mean_pred = [
            torch.mean(torch.stack([x[i] for x in outputs]), dim=0)
            for i in range(num_layers)
        ]
    
        # 使用首层输出进行后续计算（关键修改）
        main_output = mean_pred[0]
        
        # 计算损失
        l = self.loss(mean_pred, main_target)
        ...
        # 计算不确定性（保持多层计算）
        uncertainties = [
            torch.mean(-torch.sum(torch.softmax(p, 1) * torch.log(torch.softmax(p, 1) + 1e-6), dim=1)).item()
            for p in mean_pred
        ]
        ...
        # 修改点1：选择主输出层（通常为第0层）
        main_mean_pred = mean_pred[0]  # 新增关键行
        
        # 修改点2：调整axes计算方式
        axes = [0] + list(range(2, main_mean_pred.ndim))  # 使用具体层的维度
        
        # 修改点3：调整segmentation计算
        if self.label_manager.has_regions:
            predicted_segmentation_onehot = (torch.sigmoid(main_mean_pred) > 0.5).long()
        else:
            output_seg = main_mean_pred.argmax(1)[:, None]
            predicted_segmentation_onehot = torch.zeros(main_mean_pred.shape, device=self.device)
    ...
    def _set_mc_dropout_mode(self, enable=True):
        """递归设置所有dropout层的MC模式"""
        def apply_mc_mode(module):
            if isinstance(module, (nn.Dropout, nn.Dropout2d, nn.Dropout3d)):
                module.train() if enable else module.eval()
                if enable:
                    module.p = self.mc_dropout_prob  # 统一设置概率
        self.network.apply(apply_mc_mode)

    def _uncertainty_regularization(self, output):

        # print("\n=== 输出结构调试信息 ===")
        # print(f"原始输出类型：{type(output)}")
        # if isinstance(output, (tuple, list)):
        #     print(f"输出长度：{len(output)}")
        #     for i, item in enumerate(output[:3]):  # 只打印前3个元素
        #         print(f"元素{i}类型：{type(item)}")
        #         if isinstance(item, torch.Tensor):
        #             print(f"元素{i}形状：{item.shape}")
        #         else:
        #             print(f"元素{i}内容示例：{str(item)[:100]}...")
        # print("=======================\n")

        """处理深度监督的列表输出结构"""
        # 当启用深度监督时，output是包含多个层次预测的列表
        if isinstance(output, list):
            # 第一个元素是最高分辨率的主输出
            main_output = output[0]
            # 验证主输出形状 (batch, channels, x, y, z)
            if main_output.dim() != 5:
                raise ValueError(f"主输出维度异常，预期5维，实际得到{main_output.dim()}维")
        elif isinstance(output, torch.Tensor):
            main_output = output
        else:
            raise TypeError(f"无法处理的输出类型：{type(output)}")
    
        # 计算预测熵
        prob = torch.softmax(main_output, 1)
        entropy = -torch.sum(prob * torch.log(prob + 1e-6), dim=1)
        
        return 0.1 * torch.mean(entropy)
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        # 处理验证不确定性
        if 'uncertainty' in outputs_collated:
            val_uncertainty = np.mean(outputs_collated['uncertainty'])
            self.logger.log('val_uncertainty', val_uncertainty, self.current_epoch)
            # self.print_to_log_file(f'Val Uncertainty: {val_uncertainty:.4f}')
    ...
    def on_epoch_end(self):
        ...
        self.print_to_log_file(
            'train_uncert',
            np.round(self.logger.my_fantastic_logging['train_uncertainty'][-1], decimals=4)
        )
        self.print_to_log_file(
            'val_uncert',
            np.round(self.logger.my_fantastic_logging['val_uncertainty'][-1], decimals=4)
        )
        
        # 修改绘图调用（移除extra_metrics参数）
        if self.local_rank == 0:
            self.logger.plot_progress_png(self.output_folder)
            
        # 记录最佳不确定性指标
        current_train_uncert = self.logger.my_fantastic_logging['train_uncertainty'][-1]
        current_val_uncert = self.logger.my_fantastic_logging['val_uncertainty'][-1]
        
        if self.logger.my_fantastic_logging['best_train_uncert'] is None or \
           current_train_uncert < self.logger.my_fantastic_logging['best_train_uncert']:
            self.logger.my_fantastic_logging['best_train_uncert'] = current_train_uncert
        
        if self.logger.my_fantastic_logging['best_val_uncert'] is None or \
           current_val_uncert < self.logger.my_fantastic_logging['best_val_uncert']:
            self.logger.my_fantastic_logging['best_val_uncert'] = current_val_uncert
                
       
        # 检查 Pseudo dice 是否全为 0
        pseudo_dice = [np.round(i, decimals=4) for i in self.logger.my_fantastic_logging['dice_per_class_or_region'][-1]]
        if all(value == 0 for value in pseudo_dice):
            # 如果当前 epoch 的 Pseudo dice 全为 0，则记录到连续计数器中
            if not hasattr(self, 'zero_pseudo_dice_count'):
                self.zero_pseudo_dice_count = 0
            self.zero_pseudo_dice_count += 1

            # 如果1 个 checkpoint 要求的epoch内 的 Pseudo dice 全为 0，则终止进程
            if (self.zero_pseudo_dice_count + 1) == self.save_every:
                print(
                    f"Error: Pseudo dice has been 0 for consecutive epochs. Training terminated. current_epoch={self.current_epoch}")
                exit(1)  # 终止进程
        else:
            # 如果 Pseudo dice 不全为 0，则重置计数器
            self.zero_pseudo_dice_count = 0
        ...
        if self.local_rank == 0:
            if not self.disable_checkpointing:
                ...
                checkpoint = {
                    ...
                    'best_train_uncert': self.logger.my_fantastic_logging['best_train_uncert'],
                    'best_val_uncert': self.logger.my_fantastic_logging['best_val_uncert'],
    ...
    def load_checkpoint(self, filename_or_checkpoint: Union[dict, str]) -> None:
        ...
        self.logger.my_fantastic_logging['best_train_uncert'] = checkpoint.get('best_train_uncert', None)
        self.logger.my_fantastic_logging['best_val_uncert'] = checkpoint.get('best_val_uncert', None)
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        with multiprocessing.get_context("spawn").Pool(default_num_processes) as segmentation_export_pool:
            ...
            for i, k in enumerate(dataset_val.keys()):
                ...
                data, seg, properties, previous_data = dataset_val.load_case(k)
                ...
                current_properties = properties['current_properties']
                # this needs to go into background processes
                results.append(
                    segmentation_export_pool.starmap_async(
                        export_prediction_from_logits, (
                            (prediction, current_properties, self.configuration_manager, self.plans_manager,
