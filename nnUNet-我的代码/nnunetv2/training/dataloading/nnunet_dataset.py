...
class nnUNetDataset(object):
    def __init__(self, folder: str, case_identifiers: List[str] = None,
        ...
        #### 改后，但是感觉没必要，因为原代码也能正常运行
        # if len(case_identifiers) <= num_images_properties_loading_threshold:
        #     for i in self.dataset.keys():
        #         # 修改这里：加载 pkl 文件并调整为新的格式
        #         properties = load_pickle(self.dataset[i]['properties_file'])
        #         combined_properties = {
        #             'current_properties': properties.get('current_properties', {}),
        #             'previous_properties': properties.get('previous_properties', {})
        #         }
        #         self.dataset[i]['properties'] = combined_properties
        #### 改后，但是感觉没必要，因为原代码也能正常运行
    ...
    def load_case(self, key):
        ...
        if 'open_data_file' in entry.keys():
            ...
            data = np.load(entry['data_file'])['current_data']

        # 加载 previous_data
        if 'open_previous_data_file' in entry.keys():
            previous_data = entry['open_previous_data_file']
        elif isfile(entry['data_file'][:-4] + "_previous.npy"):
            previous_data = np.load(entry['data_file'][:-4] + "_previous.npy", 'r')
            if self.keep_files_open:
                self.dataset[key]['open_previous_data_file'] = previous_data
        else:
            npz_file = np.load(entry['data_file'])
            previous_data = npz_file['previous_data']
        ...
        if 'open_seg_file' in entry.keys():
            ...
            seg = np.load(entry['data_file'])['current_seg']
        ...
        return data, seg, entry['properties'], previous_data
