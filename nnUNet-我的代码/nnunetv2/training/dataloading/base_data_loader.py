...
class nnUNetDataLoaderBase(DataLoader):
    def __init__(self,
        ...
        self.data_shape, self.seg_shape, self.previous_data_shape = self.determine_shapes()
    ...
    def determine_shapes(self):
        # load one case
        data, seg, properties, previous_data = self._data.load_case(self.indices[0])
        # print("@@" * 50)

        num_color_channels_current = data.shape[0]
        num_color_channels_previous = previous_data.shape[0]
        data_shape = (self.batch_size, num_color_channels_current, *self.patch_size)
        previous_data_shape = (self.batch_size, num_color_channels_previous, *self.patch_size)
        seg_shape = (self.batch_size, seg.shape[0], *self.patch_size)

        return data_shape, seg_shape, previous_data_shape
