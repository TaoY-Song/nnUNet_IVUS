...
class DefaultPreprocessor(object):
    def __init__(self, verbose: bool = True):
        ...
        self.results = []  # 在类中维护一个累积的 results 列表
        """
    ...
    def run_case_npy(self, data: np.ndarray, seg: Union[np.ndarray, None], properties: dict,
        ...
        if self.verbose:
            print(
                f'\r\nold shape: {old_shape}, new_shape: {new_shape}, old_spacing: {original_spacing}, \r\n'
    ...
    def run_case_save(self, output_filename_truncated: str, image_files: List[str], seg_file: str,
        ...
        # 将文件名、data、seg 和 data_properties 保存到数组中
        results.append({
    ...
    def run(self, dataset_name_or_id: Union[int, str], configuration_name: str, plans_identifier: str,
        ...
        manager = SyncManager()
        manager.start()
        results = manager.list()  # 使用 SyncManager 提供的共享列表
        ...
        #     print(f"Result {i + 1}: Filename = {result['filename']}, Data shape = {result['data'].shape}, Seg shape = {result['seg'].shape}")
        print("-" * 50)
        print("Default_preprocessing finished.\r\nBegin saving data.")
        print("-" * 50)

        # 将共享的 results 列表转换为普通列表
        ...
        for prefix, file_group in grouped_files.items():
            for i, (time_index, result) in enumerate(file_group):
                ...
                output_filename = join(output_directory, f"{prefix}_{time_index}")
                
                # 获取前一帧的数据
                if i == 0:
                    # 如果是第一个文件，将当前帧的 data 作为前一帧的数据
                    previous_data = current_data
                    previous_properties = current_properties
                    print(f"Saved {current_filename} and previous frame {current_filename} to {output_filename}.npz and {output_filename}.pkl")
                else:
                    # 否则，使用前一帧的数据
                    previous_data = file_group[i - 1][1]['data']
                    previous_properties = file_group[i - 1][1]['data_properties']
                    print(f"Saved {current_filename} and previous frame {file_group[i - 1][1]['filename']} to {output_filename}.npz and {output_filename}.pkl")
                ...
                write_pickle(combined_properties, output_filename + '.pkl')
